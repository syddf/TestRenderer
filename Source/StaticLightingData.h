...
public:
	...
	void GetStaticLightingVertexVec(std::vector<StaticLightingVertex>& VertexVec) const
	{
		VertexVec.reserve(Indices.size());
		for (size_t i = 0; i < Indices.size(); i++)
		{
			StaticLightingVertex Vert;
			Vert.Position = Vertices[Indices[i]].Position;
			Vert.Normal = Vertices[Indices[i]].Normal;
			Vert.TexCoord0 = Vertices[Indices[i]].TexCoord0;
			Vert.TexCoord1 = Vertices[Indices[i]].TexCoord1;
